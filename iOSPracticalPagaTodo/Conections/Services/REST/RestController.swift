...
public class RestController : NSObject, URLSessionDelegate {
    ...
    private static let kStringType = "text/plain"
    ...
    private func dataTask(relativePath: String?, httpMethod: String, accept: String, payload: Data?, options: RestOptions, stringContent: Bool, callback: @escaping (Result<Data>, HTTPURLResponse?) -> ()) throws {
        let restURL: URL;
        ...
        if let payloadToSend = payload {
            ...
            if stringContent {
                request.setValue(RestController.kStringType, forHTTPHeaderField: RestController.kContentType)
            }else {
    ...
    private func makeCall<T: Deserializer>(_ relativePath: String?, httpMethod: String, payload: Data?, responseDeserializer: T, options: RestOptions, stringContent: Bool = false, callback: @escaping (Result<T.ResponseType>, HTTPURLResponse?) -> ()) {
        do {
            try dataTask(relativePath: relativePath, httpMethod: httpMethod, accept: responseDeserializer.acceptHeader, payload: payload, options: options, stringContent: stringContent) { (result, httpResponse) -> () in
                do {
    ...
    public func post(_ json: JSON, at relativePath: String? = nil, options: RestOptions = RestOptions(), withDataString : Bool = true, callback: @escaping (Result<JSON>, HTTPURLResponse?) -> ()) {
        do {
            //            let payload = try json.makeData()
            var payload : Data
            if withDataString {
                payload = json.makeDataString()
            }else {
                payload = try json.makeData()
            }
            makeCall(relativePath, httpMethod: RestController.kPostType, payload: payload, responseDeserializer: JSONDeserializer(), options: options, stringContent: withDataString, callback: callback)
        } catch {
            callback(.failure(error), nil)
        }
    }
    ...
    /// - parameter options: An **optional** parameter of a `RestOptions` struct for this call.
    /// - parameter callback: Called when the network operation has ended, giving back a Boxed `Result<T.ResponseType>` and a `NSHTTPURLResponse?` representing the response from the server. Note: The callback is **NOT** called on the main thread.
    public func post<T: Deserializer>(_ json: JSON, withDeserializer responseDeserializer: T, at relativePath: String? = nil, options: RestOptions = RestOptions(), withDataString : Bool = true, callback: @escaping (Result<T.ResponseType>, HTTPURLResponse?) -> ()) {
        do {
//            let payload = try json.makeData()
            var payload : Data
            if withDataString {
