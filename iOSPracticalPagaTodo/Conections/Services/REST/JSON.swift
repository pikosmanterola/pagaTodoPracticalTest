...
internal extension JSON {
    init(fromData data: Data) throws {
        let cleanData = String(data: data, encoding: String.Encoding.isoLatin1)
        let modifiedDataInUTF8Format = cleanData?.data(using: String.Encoding.utf8)
        let json = try JSONSerialization.jsonObject(with: modifiedDataInUTF8Format!, options: .mutableContainers)
...
//        debugPrint(("d=" + JSON.init(rawValue: raw).description).utf8)
//        return Data(("d=" + JSON.init(rawValue: raw).description).utf8)
        var str = "d="
        if let jsonObj = self.raw as? [String : JSONValue] {
//            print(jsonObj.description)
            str = str + jsonObj.description
